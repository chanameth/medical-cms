...
export default function TableTopic16(props) {
  ...
  const getData = async () => {
    const result = await axios(
      'http://rm2drr.org/api/topic/topic16Lab/query',
    );
  ...
  const updateData = async(newData) => {
    axios.put('http://rm2drr.org/api/topic/topic16Lab/updateOne', {
      dataUpdate:newData ,
      where: {id:newData.id}
    })
  ...
  const removeData = async(newData) => {
    axios.delete('http://rm2drr.org/api/topic/topic16Lab/deleteOne', {
      where: {id:newData.id}
    })
    .then(function (response) {
      console.log(response);
  ...
  const addData = async(newData) => {
    axios.post('http://rm2drr.org/api/topic/topic16Lab/create', {
      dataCreate:newData
    })
    .then(function (response) {
      console.log(response);
  ...
  const columns = [
    ...
    initialEditValue: props.topic16_1.data.length > 0 ?props.topic16_1.data[props.topic16_1.data.length-1].id+1:""},
    { title: 'รหัส 5 หลัก', field: 'code_id'},
    { title: 'เขต', field: 'district_id'},
    { title: 'ศวก.', field: 'hub_id'},
    { title: 'ห้องปฎิบัติการ', field: 'laboratory_name'},
